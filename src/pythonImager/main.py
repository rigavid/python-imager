...
def fusionImages(img, base_img, pos:point=[0, 0]):
    ...
    try:
        ...
        return base_img
def new_img(dimensions=None, background:COL=COL.white, name:str="NewImg"):
...
class Image:
    class Mouse:
        ...
        def get(event:str, x:int, y:int, flags:list, _):
    ...
    def setMouseCallback(self, funct:function, params=None) -> None:
        '''event, x, y, flags, params -> None'''
        self.callbacks.append([funct, params])
    class Trackbar:
        def defImg(self, img) -> None: self.img = img
        def __init__(self, name:str="TrackBar", coos:list[point]=[[100, 200], [500, 300]], min=0, max=100, val=0, col1:COL=COL.red, col2:COL=COL.darkRed, col3:COL=COL.green, font=None, thickness=3, lineType:int=2, fontSize=1) -> None:
        ...
        def draw(self) -> None: ## TODO Write value, minimum and maximum somewhere
            ...
            X, Y = self.img.textSize(self.name, font=self.font, fontSize=self.fs, thickness=self.tk)
    ...
    def trackbar(self, *args, **kwargs) -> Trackbar:
        ...
        return trkb
    def remove_trackbar(self, trkb:Trackbar) -> Trackbar | int:
        try: return self.trackbars.pop(self.trackbars.index(trkb))
        except: return -1
    class Button:
        def __init__(self, name:str="Button", coos:list[point]=[[100,100], [300, 200]], font=None, col1:COL=COL.red, col2:COL=COL.darkRed, thickness=3, lineType:int=2, fontSize=1) -> None:
            self.name, self.coos = name, coos
        ...
        def on_click(self, funct:function, params=None) -> None:
    ...
    def button(self, *args, **kwargs) -> Button:
        ...
        return bttn
    def remove_button(self, bttn:Button) -> Button | int:
        try: return self.buttons.pop(self.buttons.index(bttn))
        except: return -1
    ################################################################
    def new_image(self=None, dimensions=RES.resolution, background:COL=COL.white) -> np.array:
        '''New Image'''
        return np.full([round(v) for v in dimensions[::-1]]+[3], background[::-1], np.uint8)
    def __init__(self, name:str="python-Image", img=None, disable_callback:bool=False) -> None:
    ...
    def size(self, reverse:bool=False) -> list[int]:
        '''Returns Image's size (reverse True means [y,x] whereas False means [x,y])'''
        return [len(self.img[0]), len(self.img)][::-1 if reverse else 1]
    def build(self):
    ...
    def open_img(self, path:str) -> None:
    ...
    def save_img(self, path:str='', fileName=None) -> None:
        '''Saves file'''
    ...
    def Show(self, wait:number=1, destroy:bool=False, built_in_functs:bool=True, QtGui:bool=False) -> int:
    ...
    def move(self, pos:point) -> None: cv2.moveWindow(self.name, *pos)
    def toggleFullscreen(self) -> None: self.fullscreen = not self.fullscreen
    def setFullscreen(self, fullscreen:bool) -> None: self.fullscreen = fullscreen
    ...
    def line(self, p1, p2, colour=COL.black, thickness=1, lineType:int=0) -> None:
        '''Draws a line on the Image'''
        cv2.line(self.img, [round(p) for p in p1], [round(p) for p in p2], colour[::-1], round(thickness), lineTypes[lineType%len(lineTypes)])
    def rectangle(self, p1:point, p2:point, colour=COL.black, thickness=1, lineType=0) -> None:
    ...
    def circle(self, ct:point, radius:number=10, colour:COL=COL.black, thickness=1, lineType:int=0) -> None:
    ...
    def textSize(self, text:str, font:str="default", thickness=1, fontSize=1) -> tuple:
        ...
        except OSError as e: print(f"Couldn't load font: <{fontPath}>"); raise e
        xa, ya, xb, yb = PIL_ImageDraw.Draw(PIL_Image.fromarray(self.img)).multiline_textbbox((0,0), text, font, stroke_width=thickness)
        return (round(diff(xa, xb)),round(diff(ya, yb)))
    def text(self, text:str, pt:point, col:COL=COL.black, thickness=1, fontSize=1, angle=0, font:str="default", anchor:str="mm", align:str="center", fillBackround=None) -> None:
        """
        ...
        """
        ## TODO  Sometimes, last letter not written, found at Image.Trackbar, but no clue there.
        ## FIXME Sometimes, last letter not written, found at Image.Trackbar, but no clue there.
        ## BUG   Sometimes, last letter not written, found at Image.Trackbar, but no clue there.
        if font in ["", None]: font = "default"
        fontPath = f"{fonts_path}/unifont.ttf" if font=="default" else font
        try: font = PIL_ImageFont.truetype(fontPath, fontSize)
        except OSError as e:
    ...
    def write_centered(self, text:str, ct:point, font:str, colour:COL=COL.red, thickness:int=1, fontSize:int=1, lineType:int=0) -> None:
...
class Layout:
    class Frame:
        def __init__(self, name:str, img:Image=new_img(background=COL.white), pos:point=[0,0]) -> None:
            self.name, self.img, self.pos = name, img.copy(), pos
        def __str__(self) -> str: return self.name
    def __init__(self, img:Image=new_img(), frames:list[Frame]=[], name:str="Layout") -> None:
        self.name, self.img, self.frames = name, img.copy(), frames
        self.img.name = self.name
    def frame(self, img:Image=new_img([100, 100], COL.white), pos:point=[0,0], name=None):
        frame = self.Frame(img=img, pos=pos, name='Unnamed_frame' if name==None else name)
        self.frames.append(frame)
        return frame
    def show(self, borders:COL=False, frames:list[Frame]=None, except_frames:list[Frame]=[]) -> int:
        img = self.img.copy().img
        ...
        for frame in frames:
            self.img.img = fusionImages(frame.img.img, img, frame.pos)
...
if __name__ == "__main__":
    ...
    img.set_img(img.new_image(background=COL.new("#800080")))
    img.show(0)